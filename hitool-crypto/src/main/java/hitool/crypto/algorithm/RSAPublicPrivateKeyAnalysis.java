...
/**
 * Implementation of r s a public private key analysis functionality.
 *
...
public class RSAPublicPrivateKeyAnalysis {
	...
	public static RSAPublicKey constructRSAPublicKey(byte[] m, byte[] pubE)
			throws Exception {
		RSAPublicKeySpec pubKeySpec = new RSAPublicKeySpec(new BigInteger(m),
				new BigInteger(pubE));
	...
	public static X509Certificate ummarshalCertificate(String certFileName)
			throws Exception {
		...
		ByteArrayInputStream bais = new ByteArrayInputStream(Base64.decodeBase64(baos
				.toByteArray()));
		fis.close();
